  ...
  return (
    ...
    >
      <motion.div
        ...
        data-panel
        className={cx(
          "bg-white relative bottom-4 z-3 flex flex-col items-center overflow-hidden border border-emerald-100 max-sm:bottom-5"
        )}
        initial={false}
        animate={{
          width: showForm ? width : "auto",
          height: showForm ? height : 44,
        ...
        <div className="flex w-fit items-center gap-2">
          <AnimatePresence mode="wait">
            {showForm ? (
              ...
              >
                <ColorOrb dimension="24px" tones={{ base: "oklch(92% 0.02 142)" }} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
...
function InputForm({
  ...
  return (
    <form
      ...
      style={{ width, height, pointerEvents: showForm ? "all" : "none" }}
    >
      <AnimatePresence>
        {showForm && (
          <motion.div
          ...
          >
            <div className="flex justify-between py-1">
              <p className="text-slate-700 z-2 ml-[38px] flex items-center gap-[6px] select-none">
                AI Input
              </p>
              <button
                type="submit"
                ref={btnRef}
                className="text-slate-600 right-4 mt-1 flex -translate-y-[3px] cursor-pointer items-center justify-center gap-1 rounded-[12px] bg-transparent pr-1 text-center select-none"
              >
                <KeyHint>⌘</KeyHint>
            ...
            <textarea
              ref={textareaRef}
              placeholder="Ask about this county..."
              name="message"
              className="h-full w-full resize-none scroll-py-2 rounded-md p-4 outline-0 text-sm text-slate-700"
              required
...
function KeyHint({ children, className }: { children: string; className?: string }) {
  return (
    <kbd
      className={cx(
        "text-slate-600 flex h-6 w-fit items-center justify-center rounded-sm border px-[6px] font-sans",
        className
      )}
    >
      {children}
